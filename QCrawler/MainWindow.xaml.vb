...
Class MainWindow
    ...
    Private Function getQCodeFromFilename(filename As String) As String
        ...
        'If filename.Substring(0, 3) = "Cal" Then
        'filename = "QCS1_" & filename ' For ETH TTOF data
        ' End If
                                  ...
                                  If IO.Directory.Exists(monitoredFolder) Then 'Check if local folder exists.
                                      ...
                                      If IO.Directory.Exists(processedFolderTarget) Then
                                          If validateQCodes(QCodeString) Then
                                              Dim foundFiles As ArrayList = filterLocalFolder(QCodeString, QCodeExcludesString, monitoredFolder) 'Get a list of local files filtered by QC code, extension and not locked.
                                              If (isCalFolder(QCodeString)) Then 'Append cal files for TTOF
                                                  Dim foundCalFiles As ArrayList = filterLocalFolder(QCodeString, QCodeExcludesString, monitoredFolder & "\cal")
